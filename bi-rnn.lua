...
-----------------------------------------------------------------
Bi-Directional Recurrent Neural Networks for Tweet Classification
-----------------------------------------------------------------
...
params.model.modules[1]:add(params.rnn)
params.model.modules[1]:add(params.rnn:sharedClone())
params.model:add(nn.MaskZero(nn.JoinTable(2, 2), 1))
if params.dropout > 0 then params.model:add(nn.MaskZero(nn.Dropout(params.dropout), 1)) end
params.model:add(nn.MaskZero(nn.Linear(2 * input_size, #params.index2label), 1))
...
function train(inp, inp_rev, tar, optim_states)
	params.model:training()
	local indices = torch.randperm(#params.train_tensors)
	local input_x, input_rev_x, input_y = torch.CudaTensor(params.bsize, params.max_words), torch.CudaTensor(params.bsize, params.max_words), torch.CudaTensor(params.bsize, 1) 
	local epoch_loss, num_batches = 0, math.floor((#inp)[1] / params.bsize)
	function train_batch(batch_size)
		local feval = function(x)		
			params.gp:zero()
			local outputs = params.model:forward({input_x, input_rev_x})
			...
			params.model:backward({input_x, input_rev_x}, grads)
	...
	if rem_inputs > 0 then
		input_x, input_rev_x, input_y = torch.CudaTensor(rem_inputs, params.max_words), torch.CudaTensor(rem_inputs, params.max_words), torch.CudaTensor(rem_inputs, 1) 
		local start = (num_batches * params.bsize)
		for i = 1, rem_inputs do
			local idx = indices[start+i]
			input_x[i] = inp[idx]
			input_rev_x[i] = inp_rev[idx]
			input_y[i] = tar[idx]
		end
...
function stackSamples(x_tensors)
	local input, input_rev, target = torch.CudaTensor(#x_tensors, params.max_words), torch.CudaTensor(#x_tensors, params.max_words):fill(0), {}
	for i = 1, #x_tensors do
		input[i] = x_tensors[i][1]
		local start = 1
		for j = 1, params.max_words do
			if input[i][j] ~= 0 then break end
			start = start + 1
		end
		if start < params.max_words then
			local cur_i = start
			for j = params.max_words, start, -1 do
				input_rev[i][cur_i] = input[i][j]
				cur_i = cur_i + 1
			end
		end
		table.insert(target, x_tensors[i][2])
	end
	return input, input_rev, target
end
params.train_in, params.train_in_rev, params.train_tar = stackSamples(params.train_tensors)
params.val_in, params.val_in_rev, params.val_tar = stackSamples(params.val_tensors)
params.test_in, params.test_in_rev, params.test_tar = stackSamples(params.test_tensors)

-- get f_score and accuracy.
params.soft_max = nn.SoftMax():cuda()
function compute_performance(x_in, x_in_rev, x_tar, model)
	model:evaluate()
	local out = model:forward({x_in, x_in_rev})
...
for epoch = 1, params.num_epochs do
	...
	local loss = train(params.train_in, params.train_in_rev, params.train_tar, optim_states)
	local cur_fscore, cur_acc = compute_performance(params.val_in, params.val_in_rev, params.val_tar, params.model)
...
-- Evaluation
local final_fscore, _ = compute_performance(params.test_in, params.test_in_rev, params.test_tar, best_fscore_model)
local _, final_acc = compute_performance(params.test_in, params.test_in_rev, params.test_tar, best_acc_model)
